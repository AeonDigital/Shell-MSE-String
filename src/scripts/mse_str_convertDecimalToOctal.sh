...
#
# @example
#   mse_str_convertDecimalToOctal "161"       # converte para -> 241
#   mse_str_convertDecimalToOctal "195 173"   # converte para -> "303 255"
#   result=$(mse_str_convertDecimalToOctal "161" 1)
mse_str_convertDecimalToOctal() {
    ...
    for (( i=0; i<${#mseArrParam[@]}; i++ )); do
      if [ $mseIsValid == 1 ]; then
        if ! [[ ${mseArrParam[$i]} =~ $mseREG ]]; then
          mseIsValid=0
          mse_mmod_errorAlert "${FUNCNAME[0]}" "${lbl_convert_originalValueIsNotValidDecimal}"
        fi
      fi
    done
